  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-slate-900">Local Civic Feed</h1>
        <Link href="/issues/new" className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 transition-colors shadow-sm">
          + Report Issue
        </Link>
      </div>
        ...
        {issues.length === 0 ? (
          ...
          issues.map((issue) => (
            <div key={issue._id} className="rounded-xl border border-slate-200 bg-white p-6 transition-colors hover:border-indigo-200 shadow-sm">
              ...
              <p className="mb-6 text-slate-600 line-clamp-2">{issue.description}</p>
